...
from attn_utils import AnchorCache, FeatureInjector, QueryStore, xformers
...
class ConsistoryExtendedAttnXFormersAttnProcessor:
    ...
    def __call__(
        ...
        use_consistyle_adain = True,
        attn_v_range=[3,10],
        attn_qk_range=[5,15],
        no_attn_q = False,
        no_attn_k = False,
        use_consistyle_direct_injection = False,
        **kwargs
    ) -> torch.FloatTensor:
        ...
        if use_styled_feature_injection and feature_injector is not None and curr_unet_part == 'up' and width == 64:
            for i in range(batch_size //2, batch_size):
                ...
                if attn_qk_range[0]  <= self.attnstore.curr_iter <= attn_qk_range[1]: # key, query first pass, value sdxl vanila
                    ...
                    if use_consistyle_direct_injection:
                        other_query = query[0]
                        other_key = key[0]
        ...
        if perform_extend_attn:
            ...
            # Check if we should perform injection (with safety check for cache existence)
            do_injection = False
            if anchors_cache and anchors_cache.is_inject_mode():
                if self.place_in_unet in anchors_cache.input_h_cache and \
                   self.attnstore.curr_iter in anchors_cache.input_h_cache[self.place_in_unet]:
                    do_injection = True
            ...
            if do_injection:
                ...
                for i in range(batch_size):
                    ...
                    if use_styled_feature_injection and use_consistyle_adain:
                    ...
                    if hidden_states.dtype == torch.float16:
                        hidden_states_out = xformers.ops.memory_efficient_attention(
                            curr_q, curr_k, curr_v, 
                            op=self.attention_op, scale=attn.scale
                        )
                    else:
                        attention_probs = attn.get_attention_scores(curr_q, curr_k)
                        hidden_states_out = torch.bmm(attention_probs, curr_v)
                        
                    ex_out[start_idx:end_idx] = hidden_states_out
